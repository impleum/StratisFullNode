...
namespace Stratis.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    /// </list></para></remarks>
    public class ImplxColdStakingRule : UtxoStoreConsensusRule
    {
        /// <inheritdoc />
        ...
        public override Task RunAsync(RuleContext context)
        {
            ...
            for (int i = 2; i < coinstakeTransaction.Outputs.Count; i++)
            {
                if (scriptPubKey != coinstakeTransaction.Outputs[i].ScriptPubKey)
                {
                    ...
                    if (coinstakeTransaction.Outputs[i].ScriptPubKey == ImplxCoinstakeRule.CirrusRewardScript && !cirrusScriptFlagSeen)
                    {
